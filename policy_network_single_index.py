...
def generate_policy_case(train_case,gold_chain=[],network=None):
    ...
    new_cluster_num = 0
    actions = []
    for single,sindex,tc,tindex in train_case:
        if len(tc) == 0:
            action_probability = numpy.array([1])
        else:
            action_probability = network.predict(single,sindex,tc,tindex)[0]
    ...
    for i in indexs:
        single,sindex,tc,tindex = train_case[i]
        if len(tc) == 0:
            continue
        yield single, sindex, tc, tindex, actions[i], reward
...
def generate_policy_test(train_case,gold_chain=[],network=None):
    ...
    for single,sindex,tc,tindex in train_case:
        if len(tc) == 0:
            ...
            action_probability = list(network.predict(single,sindex,tc,tindex)[0])
